"""
============================================
ELO-BASED NHL PREDICTION - HOME ADVANTAGE ANALYSIS
Group: 1
Python Version: 3.9+
============================================

Pure ELO rating approach following Arntzen & Hvattum (2010).
Tests different home advantage values with fixed K=20 to find optimal prediction accuracy.
...
print("=" * 80)
print("ELO-BASED NHL PREDICTION - HOME ADVANTAGE ANALYSIS")
print("=" * 80)
...
print("\n[2/4] Running home advantage analysis...")

# Fixed K-factor (from previous analysis)
K_FACTOR = 20

# Home advantage values to test
HOME_ADVANTAGES = [20, 40, 50, 60, 70, 80, 90, 100]

# Store results
results = []

for home_advantage in HOME_ADVANTAGES:
    print(f"\n  Testing home advantage = {home_advantage}...")
...
print("\n" + "=" * 80)
print("HOME ADVANTAGE ANALYSIS RESULTS (K=20)")
print("=" * 80)
print("\nPrediction Accuracy by Home Advantage:")
print("-" * 80)
print(f"{'Home Adv':<15} {'Train Acc':<15} {'Test Acc':<15} {'Test Brier':<15}")
print("-" * 80)
for _, row in results_df.iterrows():
    print(f"{row['home_advantage']:<15.0f} {row['train_accuracy']:.4f} ({row['train_accuracy']*100:.2f}%)  "
          f"{row['test_accuracy']:.4f} ({row['test_accuracy']*100:.2f}%)  "
          f"{row['test_brier']:.4f}")
print("-" * 80)

# Find best home advantage
best_idx = results_df['test_accuracy'].idxmax()
best_ha = results_df.loc[best_idx, 'home_advantage']
best_acc = results_df.loc[best_idx, 'test_accuracy']

print(f"\nBest Home Advantage: {best_ha:.0f} (Test Accuracy: {best_acc:.4f} or {best_acc*100:.2f}%)")
print("=" * 80)
...
# Run simulation with best home advantage
full_df = pd.concat([train_df, test_df], ignore_index=True).sort_values('date')
full_df_with_elo, predictions = run_elo_simulation(
    full_df,
    k_factor=K_FACTOR,
...
print("\n" + "=" * 80)
print(f"DETAILED RESULTS FOR BEST MODEL (K={K_FACTOR}, Home Adv={best_ha:.0f})")
print("=" * 80)
...
results_df.to_csv('elo_home_advantage_results.csv', index=False)
print("  Saved: elo_home_advantage_results.csv")
...
test_df_final[output_cols].to_csv('elo_test_predictions_best_ha.csv', index=False)
print("  Saved: elo_test_predictions_best_ha.csv")

# Save summary metrics
summary = {
    'model': 'Pure ELO Rating Model (Arntzen & Hvattum 2010)',
    'k_factor': K_FACTOR,
    'home_advantages_tested': HOME_ADVANTAGES,
    'best_home_advantage': int(best_ha),
    'results_by_home_advantage': results_df.to_dict('records'),
...
with open('elo_home_advantage_summary.json', 'w') as f:
    json.dump(summary, f, indent=4)
print("  Saved: elo_home_advantage_summary.json")

print("\n" + "=" * 80)
print("ANALYSIS COMPLETE")
print("=" * 80)
print(f"\nK-Factor: {K_FACTOR}")
print(f"Best Home Advantage: {best_ha:.0f}")
